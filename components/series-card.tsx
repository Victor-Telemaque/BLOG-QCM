...
const SeriesCard: FunctionComponent<SeriesCardProps> = (props) => {
  ...
  return (
    <section
      className={`text-white border-l-8 ${getBorderClass(
        post.theme
      )} p-4 mb-4 rounded bg-cool-gray-900`}
    >
        ...
        if (relatedPost.id !== post.id) {
          return (
            <Link href={`./${relatedPost.id}`}>
              <a>
                <p className="mb-4">
                  <span
                    className={`rounded-full px-3 py-1 bg-${post.theme}-500 mr-2`}
                  >
                    {index + 1}
                  </span>
          ...
          return (
            <p className="mb-4">
              ...
              <span className={`text-${relatedPost.theme}-500`}>
                {relatedPost.title}
              </span>
            </p>
          );
